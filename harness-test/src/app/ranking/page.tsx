      ...
      <div className="flex-1 px-6 py-12 max-w-md mx-auto w-full">
        <AnimatedSection delay={0}>
          ...
          <p className="text-white/50 text-sm text-center mb-8">
            {total > 0 ? `${total.toLocaleString()}명이 참여했어요` : "첫 번째 참여자가 되어보세요"}
          </p>
        </AnimatedSection>
                ...
                <AnimatedSection key={type} delay={0.05 * idx}>
                  <div className="bg-white/5 rounded-2xl p-4 border border-white/10 backdrop-blur-sm">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      {idx === 0 && count > 0 && (
                        <span className="text-xs bg-brand-400/20 text-brand-400 px-2 py-0.5 rounded-full border border-brand-400/30">
                          1위
                        </span>
                      )}
                    </div>
